...
Second Note: The Dynacool PPMS has no GPIB address. When you add it, you can leave the GPIB address section blank. Note, the program connects to the Dynacool using a server. To properly connect, you need run the measurement program on the same computer as MultiVu AND the address defined in "PPMS_Instrument_Classes.py" must be updated according to you're system.
